public class Graph<T> implements GraphADT<T>
{
    ...
    {    
        //Gets index and passes it to the other removeEdge method
    ...
    {
        //Checks if index is valid and if so, it will set the
        //location of the edge to false and increment modCount
        if (indexIsValid(index1) && indexIsValid(index2))
        {
    ...
    {
        //If the number of vertices is close to the matrix size
        //the capacity will expand
       if ((numVertices + 1) == adjMatrix.length){
            expandCapacity();
        }

        //adds null vertex since a vertex was not passed in    
        vertices[numVertices] = null;
        
        //Sets all connections to false 
        //no vertex is connected to the new vertex
        for (int i = 0; i <= numVertices; i++){
            adjMatrix[numVertices][i] = false;
            adjMatrix[i][numVertices] = false;
        }   
        
        //increments number of vertices and mod count
        numVertices++;
        modCount++;
    }
    ...
    {
        //Passes vertex index to other removeVertex() method
    ...
    {  
        ...
        if (indexIsValid(index)){
            ...
            //setting vertex to next vertex
            for (int i = index; i < numVertices; i++){
                vertices[i] = vertices[i+1];
            }

            //Setting matrix value to next matrix value
            for (int i = index; i < numVertices; i++){
    ...
    public boolean isConnected() { 
        ...
        //Creates breadth first traversal iterator
        Iterator<T> it = iteratorBFS(0);
        int count = 0;
        
        //Breadth-first traversal over the matrix
        while (it.hasNext())
        {
            it.next();
            count++;
        ...
        //if count == the number of vertices - the graph is connected
    ...
    {
        //finds index when vertice == vertex
        for (int i = 0; i < numVertices; i++){
